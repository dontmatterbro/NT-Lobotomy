...
Hook.Add("item.applyTreatment", "NTLOBO.itemused", function(item, usingCharacter, targetCharacter, limb)
	if NTLOBO.Robot(targetCharacter) then
		return
	end --robotrauma comp

	if HF.HasAffliction(targetCharacter, "stasis") then
		return
	end
	...
	then
		...
		HF.GiveItem(usingCharacter, "it_orbitoclast") --give back orbitoclast
...
Hook.Add("nerveregen", function(effect, deltaTime, item, targets, worldPosition, element)
	for k, targetCharacter in pairs(targets) do
		...
		then
			if not HF.HasAffliction(targetCharacter, "lobotomy") then
...
function NTLOBO.ApplyLobotomy(targetCharacter, prevresult, prevchance)
	...
	then
		if HF.Chance(HF.GetAfflictionStrength(targetCharacter, "lobotomy") * 0.06) then
